...
RUN curl -s -L https://github.com/opencv/opencv/archive/3.1.0.zip > opencv-3.1.0.zip
RUN unzip opencv-3.1.0.zip
RUN rm opencv-3.1.0.zip
WORKDIR /app/.heroku/opencv-3.1.0
RUN cmake -D WITH_TBB=ON -D CMAKE_BUILD_TYPE=Release -D CMAKE_INSTALL_PREFIX=/app/.heroku/vendor -D BUILD_DOCS=OFF -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D BUILD_EXAMPLES=OFF .
RUN make install
WORKDIR /app/.heroku
